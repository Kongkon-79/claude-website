...
const WhatDoWeOffer = () => {
  ...
  return (
    <div className="relative container bg-[url('/assets/images/home_page/sm_bg.svg')] md:bg-[url('/assets/images/home_page/lg_bg.svg')] bg-cover bg-center bg-no-repeat h-[800px] md:h-[800px] rounded-[50px] w-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-6 md:gap-7 lg:gap-8 pt-10 md:pt-16 lg:pt-20">
        {
          missionData?.map((item)=>{
            return <div key={item?.id} className="bg-white rounded-[12px] border-[4px] border-primary p-4 md:p-5">
              <Image src={item?.img} alt={item?.title} width={1000} height={1000} className="w-[145px] md:w-[180px] h-auto object-contain mx-auto"/>
