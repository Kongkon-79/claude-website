  ...
  return (
    <div className="relative max-w-[1400px] mx-auto w-[95%] md:w-full bg-[url('/assets/images/home_page/sm_bg.svg')] md:bg-[url('/assets/images/home_page/lg_bg.svg')] bg-cover bg-center bg-no-repeat min-h-[600px] md:h-[750px] py-10 md:py-16 lg:py-20 rounded-[30px] md:rounded-[70px] mt-6 md:mt-8 lg:mt-10 xl:mt-12 overflow-hidden flex flex-col justify-center items-center">
      <div className="relative z-10 px-4 md:px-8 lg:px-12 w-full max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-8 items-stretch">
...
const EcosystemCard = ({ item }: { item: any }) => {
  return (
    <div className="bg-white rounded-[25px] md:rounded-[30px] p-4 md:p-6 flex flex-col h-full shadow-2xl transition-all duration-300 hover:translate-y-[-5px]">
      {/* Top Image */}
      ...
      {/* Content */}
      <div className="px-1 md:px-2 flex flex-col h-full">
        <h3 className="text-primary font-dagger font-bold text-sm md:text-xl lg:text-2xl mb-4 md:mb-6 leading-tight">
          {item.title}
        </h3>
        
        <ul className="flex flex-col gap-3 md:gap-4 flex-grow">
          {item.features.map((feature: string, idx: number) => (
            <li key={idx} className="flex items-start gap-2 md:gap-3">
              <CircleCheck className="w-4 h-4 md:w-5 md:h-5 text-black shrink-0 mt-0.5" />
              <p className="text-black text-[11px] md:text-sm lg:text-base font-bold leading-tight">
                {feature}
              </p>
            </li>
          ))}
